...
public:
    ...
    CurveFittingVertex2(): Vertex(3) {}  // abc: 三个参数， Vertex 是 3 维的
    virtual std::string TypeInfo() const { return "abc"; }
};
...
class CurveFittingEdge2: public Edge
{
    public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    CurveFittingEdge2( double x, double y ): Edge(1,1, std::vector<std::string>{"abc"}) {
        x_ = x;
        y_ = y;
    }
    // 计算曲线模型误差
    ...
    {
        //Vec3 abc = verticies_[0]->Parameters();
        //double y =  abc(0)*x_*x_ + abc(1)*x_ + abc(2) ;
    ...
    /// 返回边的类型信息
    virtual std::string TypeInfo() const override { return "CurveFittingEdge2"; }
public:
    double x_,y_;  // x 值， y 值为 _measurement    
};
...
int main()
{
    double a=1.0, b=2.0, c=1.0;         // 真实参数值
    int N = 1000;                          // 数据点
    double w_sigma= 10.;                 // 噪声Sigma值

    std::default_random_engine generator;
    std::normal_distribution<double> noise(0.1,w_sigma);

    // 构建 problem
    Problem problem(Problem::ProblemType::GENERIC_PROBLEM);
    shared_ptr< CurveFittingVertex2 > vertex(new CurveFittingVertex2());

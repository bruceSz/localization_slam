...
void CreatePointsLines(Points& points, Lines& lines)
{
    ...
    if(!f) {
        std::cout << "f open failed." << std::endl;
    }
...
int main(){
    ...
    // 建立keyframe文件夹
    mkdir("keyframe", 0777);
    std::cout << "create keyframe dir succed." << std::endl;

    // 生成3d points
    Points points;
    Lines lines;
    CreatePointsLines(points, lines);
    std::cout << "create points lines succed." << std::endl;
    ...
    for (float t = params.t_start; t<params.t_end;) {
        std::cout<< "Gen imu data and imu  noise data" << std::endl;

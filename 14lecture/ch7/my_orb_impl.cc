...
void computeORB(const cv::Mat& img, const std::vector<cv::KeyPoint>& kps, 
    ...
    for(auto &kp: kps) {
        ...
        for ( int i=0; i< 8; i++) {
            ...
            for(int k=0; i< 32; k++) {
                ...
                if (img.at<uchar>(pp.y, pp.y) < img.at<uchar>(qq.y,qq.x) ) {
                    d |= 1<< k;
                }
            }
            desc[i] = d;
    ...
    cout << "bad/total: " << bad_point << "/"  << kps.size()<< endl;
}


void BfMatch( const std::vector<DescType>& desc1, const std::vector<DescType>& desc2,
